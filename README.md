...
girlfriend-agent 可以作为 **Claude Code 插件** 使用 — skills + 斜杠命令 + MCP 工具 + session hooks，一体化集成。
...
├── .mcp.json                      # MCP 服务器配置（stdio 传输）
├── skills/                        # 7 个技能（SKILL.md + frontmatter）
│   ├── gf-intro/                  # 介绍技能（session start 自动注入）
│   ├── gf-chat/                   # 聊天技能
│   ├── gf-status/                 # 状态查看技能
│   ├── gf-evolve/                 # 进化周期技能
│   ├── gf-memory/                 # 记忆管理技能
...
├── agents/                        # Agent 定义
│   └── girlfriend-persona.md      # 人格化回复 Agent
├── hooks/                         # Session hooks
│   ├── hooks.json                 # Hook 配置
│   ├── session-start              # 启动时注入上下文
│   └── run-hook.cmd               # Windows/Unix 双平台脚本
└── src/                           # 核心引擎代码
```
...
1. 将本项目添加为 Claude Code 插件（在设置中指定路径）
2. MCP 服务器通过 `.mcp.json` 自动启动
3. Session 启动时，`gf-intro` 技能内容自动注入为上下文
4. 使用斜杠命令：

| 命令 | 功能 |
|---|---|
| `/gf-chat` | 处理聊天消息，返回人格化上下文 |
| `/gf-status` | 查看关系等级、亲密度、属性 |
| `/gf-evolve` | 执行进化周期 |
| `/gf-memory` | 存储/搜索/强化/衰减记忆 |
| `/gf-persona` | 查看/修改人设或应用模板 |
| `/gf-graph` | 管理情景知识图谱 |

5. 所有 26 个 MCP 工具均可直接调用（带 `_girlfriend` 后缀避免冲突）
6. `girlfriend-persona` Agent 可用于生成角色驱动的回复

#### 方式二：仅 MCP Server

如果只需要 MCP 工具而不需要 skills/commands，在项目根目录 `.mcp.json` 已配置好，无需额外设置。也可手动添加到 `~/.claude/settings.json`：
...
所有工具以 `_girlfriend` 后缀命名，避免与其他插件冲突：

| 分类 | 工具 | 说明 |
|---|---|---|
| **对话** | `chat_girlfriend` | 处理消息，注入人格 + 记忆 + 去AI味指令 |
| **状态** | `status_girlfriend`, `health_girlfriend` | 关系状态 + 引擎健康检查 |
| **人设** | `persona_get_girlfriend`, `persona_update_girlfriend`, `persona_apply_template_girlfriend` | 查看/修改/切换人设模板 |
| **记忆** | `memory_update_girlfriend`, `memory_search_girlfriend`, `memory_reinforce_girlfriend`, `memory_decay_girlfriend`, `memory_emotion_trend_girlfriend` | 存储/搜索/强化/衰减向量记忆 |
| **图谱** | `graph_add_entity_girlfriend`, `graph_add_relation_girlfriend`, `graph_add_event_girlfriend`, `graph_search_girlfriend`, `graph_timeline_girlfriend`, `graph_batch_build_girlfriend`, `graph_stats_girlfriend` | 情景知识图谱操作 |
| **进化** | `evolve_run_girlfriend`, `evolve_direction_girlfriend`, `evolve_endings_girlfriend`, `evolve_progress_girlfriend`, `evolve_history_girlfriend`, `evolve_revert_girlfriend`, `evolve_revert_to_girlfriend` | 进化周期 + 回退 |
| **回退** | `rollback_girlfriend` | 全状态回退到任意 git 提交 |

### Skills 说明

| Skill | 触发场景 | 核心功能 |
|---|---|---|
| `gf-intro` | Session 启动自动注入 | 插件介绍、工具列表、快速入门 |
| `gf-chat` | 用户要与角色聊天 | 调用 `chat_girlfriend`，选择交互类型和注入等级 |
| `gf-status` | 查看关系状态 | 调用 `status_girlfriend`，展示属性条和去AI味评分 |
| `gf-evolve` | 触发进化/查看方向/回退 | 调用 `evolve_*_girlfriend` 系列工具 |
| `gf-memory` | 存储/搜索/强化记忆 | 调用 `memory_*_girlfriend` 系列工具 |
| `gf-persona` | 查看/修改人设/应用模板 | 调用 `persona_*_girlfriend` 系列工具 |
| `gf-graph` | 管理情景图谱 | 调用 `graph_*_girlfriend` 系列工具 |
...
├── .mcp.json                         # MCP 服务器配置（stdio）
├── engine_server.py                  # FastAPI 入口 + 生命周期 + asyncio.Lock
├── mcp_server.py                     # MCP 适配器（26工具，stdio传输）
├── core/
...
├── api/
│   ├── chat_router.py                # 聊天端点（ChatService Lock 保护变更）
│   ├── evolve_router.py              # 进化端点（Lock + 纯读端点文档化）
│   ├── graph_router.py               # 图谱端点（Pydantic 模型 + 幂等操作）
│   ├── memory_router.py              # 记忆端点
│   ├── persona_router.py             # 人设 CRUD
│   ├── rollback_router.py            # 通过 StateManager 回退
│   └── status_router.py              # 关系状态
├── templates/                        # 6个 人设 JSON 模板
├── prompts/                          # 7个 等级 prompt 模板
├── endings/                          # 56种结局描述库
├── skills/                           # 7个技能（SKILL.md + frontmatter）
│   ├── gf-intro/                     # 启动自动注入
│   ├── gf-chat / gf-status / gf-evolve / gf-memory / gf-persona / gf-graph
│   └── scripts/                      # CLI 桥接脚本（备用接口）
├── commands/                         # 6个斜杠命令
├── agents/                           # 1个 Agent（girlfriend-persona.md）
├── hooks/                            # session-start hook + run-hook.cmd
└── tests/                            # 612个测试
```
...
如果只需要 MCP 工具而不需要 skills/commands，项目根目录 `.mcp.json` 已配置好，无需额外设置。也可手动添加到 `~/.claude/settings.json`：
...
| **Phase 3.6** | Claude Code 插件化（skills + commands + agents + hooks） | ✅ 完成 |
